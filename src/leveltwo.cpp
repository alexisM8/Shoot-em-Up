...
LevelTwo::LevelTwo(std::shared_ptr<Engine::Context> &context, int score)
: m_context(context), m_score(score), m_playerDir({0.0f,0.0f}),m_enemyVelocity(0.2f), m_enemyDir({m_enemyVelocity, 0.0f}), m_isPasued(false), m_playerVelocity(0.5f), m_missileVelocity(1.5f), m_seeMissile(false){
    srand(time(nullptr));
}
...
void LevelTwo::Update(sf::Time deltatime){
    if(m_score >= 800){
        m_context->m_states->add(std::make_unique<WinOrLose>(m_context, true, m_score, 2), true);
    }
